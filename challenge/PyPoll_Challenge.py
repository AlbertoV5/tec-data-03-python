...
with open(file_to_save, "w") as txt_file:
    ...
    largest_county = list(county_votes.keys())[0]
    for county in county_votes:
        # 6b: Retrieve the county vote count.
        vote_count = county_votes[county]
        # 6c: Calculate the percentage of votes for the county.
        percentage = 100 * vote_count / total_votes
        # 6d: Print the county results to the terminal.
        results = (f"{county}: {percentage:.1f}% ({vote_count:,})\n")
        print(results, end = "")
        # 6e: Save the county votes to a text file.
        txt_file.write(results)
        # 6f: Write an if statement to determine the winning county and get its vote count.
        if county_votes[county] > county_votes[largest_county]:
            largest_county = county
            largest_county_voter_turnout = county_votes[county]
